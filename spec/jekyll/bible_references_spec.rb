...
RSpec.describe Jekyll::BibleReferences do
  ...
  let(:custom_url02) { find_by_title(site.pages, "custom URL 02") }
  let(:custom_url01) { find_by_title(site.pages, "custom URL 01") }
  ...
  context "when jekyll-mentions is set to false" do
    it "does not replace the @TestUser with the link to @TestUser" do
      expect(disabled_mentioning_page.output).not_to include(result)
    end

    it "leaves other pages in the site alone and mention as normal" do
  ...
  context "when the jekyll-mentions is overridden on a single file" do
    ...
    context "when overriden in pattern 'jekyll-mentions: custom_url'" do
      it "replaces the mentions with the link in that specific file" do
        expect(custom_url01.output).to include(custom_url_result)
      end

      it "does not include the default URL" do
        expect(custom_url01.output).not_to include(result)
      end
    ...
    context "when overriden in pattern 'jekyll-mentions.base_url: custom_url'" do
      it "replaces the mentions with the link in that specific file" do
        expect(custom_url02.output).to include(custom_url_result)
      end

      it "does not include the default URL" do
        expect(custom_url02.output).not_to include(result)
      end
